...
class ClassConditionalAE(nn.Module):
    """
    用于消融实验的 AutoEncoder（AE）版本：
        - encoder: x -> z (连续 latent)
        - decoder: z -> x_rec
    不使用 VQ、不使用 codebook、不做量化

    设计目标：
        - encoder / decoder 结构与 ClassConditionalVQVAE 完全一致
        - forward 接口一致，便于直接替换跑消融
    """

    def __init__(
        self,
        input_dim: int,
        n_classes: int,                # 保留，仅用于接口一致
        latent_dim: int = 32,
...
def train_ae_on_cells(
    ...
    """
    用于消融实验的 AE 训练函数：
        - 与 train_vqvae_on_cells 流程完全一致
        - 去除 VQ / codebook / usage 正则
    """
    if cfg is None:
        cfg = VQVAETrainingConfig()

    device = cfg.device
    print(f"[AE] 训练设备: {device}")
    ...
    print(f"[AE] 数据: total={n_total}, train={n_train}, val={n_val}")
    ...
    model = ClassConditionalAE(
        input_dim=input_dim,
        n_classes=n_classes,          # 仅用于接口一致
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        for batch_x, batch_y in train_loader:
            ...
            batch_y = batch_y.to(device)   # 不使用，仅保持接口一致

            optimizer.zero_grad()

            x_rec, recon_loss = model(batch_x, batch_y)
            recon_loss.backward()
            optimizer.step()
        ...
        print(
            f"[AE] Epoch {epoch:03d}/{cfg.max_epochs} | "
        ...
        if val_total < best_val_loss - 1e-5:
            ...
            if patience_counter >= cfg.early_stop_patience:
                print(
                    f"[AE] 早停触发: 连续 {cfg.early_stop_patience} 轮无提升，"
                    f"使用第 {best_epoch} 轮的参数 (val_recon={best_val_loss:.6f})"
                )
                break
...
def encode_cells_ae(
    ...
    """
    批量编码 AE latent，用于后续聚类 / metacell 构建
    返回:
        Z_all: (N_cells, latent_dim)
    """
    model.eval()
    ...
    for batch_x, batch_y in loader:
        ...
        z = model.encode(batch_x, batch_y)   # ← 正确调用
        Z_list.append(z.cpu())
...
def run_full_pipeline_11way10shot():
    ...
    '''
    new_metacell_ids = y_all
    metacell_centroids = Z_all
    cid2new = y_all
