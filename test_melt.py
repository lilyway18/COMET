...
def load_user_data() -> Tuple[np.ndarray, np.ndarray, Dict[int, str]]:
    ...
    h5ad_path = "./issaac_seq/rna.h5ad"
    label_col = "x"
    label_csv_path = "./issaac_seq/Label.csv"
...
class ClassConditionalVQVAE(nn.Module):
    """
    实际已改为 Global-Codebook VQ-VAE
    （接口保持 class-conditional 兼容）
    """
    ...
    def _quantize_auto(self, z_e):
        ...
        best_zq = torch.zeros_like(z_e)
        best_idx = torch.zeros(B, dtype=torch.long, device=device)
        best_k = torch.zeros(B, dtype=torch.long, device=device)

        for k, emb in enumerate(self.codebooks):
            E = emb.weight  # (Kk, D)

            z_norm2 = (z_e ** 2).sum(1, keepdim=True)
            e_norm2 = (E ** 2).sum(1).unsqueeze(0)
            dist = z_norm2 + e_norm2 - 2.0 * z_e @ E.t()

            dmin, idx = dist.min(dim=1)
            mask = dmin < best_dist

            best_dist[mask] = dmin[mask]
            best_zq[mask] = E[idx[mask]]
            best_idx[mask] = idx[mask]
            best_k[mask] = k

        return best_zq, best_k, best_idx

    def forward(self, x, y=None):
        z_e = self.encoder(x)

        z_q, codebook_id, code_idx = self._quantize_auto(z_e)
        ...
        return x_rec, vq_loss, codebook_id, code_idx
    ...
    def encode_quantized(
        ...
        """
        z_e = self.encoder(x)
        z_q, code_idx_local, _ = self._quantize_auto(z_e)
...
def train_vqvae_on_cells(
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        for batch_x, batch_y in train_loader:
            ...
            usage_loss = usage_loss.float().mean()
...
def encode_cells_vq(
    ...
    with torch.no_grad():
        for batch_x, batch_y in loader:
            ...
            offsets = torch.zeros_like(batch_y, device=device)  # (B,)
...
def encode_cells(
    X_all, model, gene_scaler, batch_size=1024, device=None, use_quantized=True
):
    model.eval()
    device = device or DEFAULT_DEVICE
    model.to(device)

    X_scaled = X_all.astype(np.float32)  # 直接用
    X_t = torch.from_numpy(X_scaled)
    ...
    with torch.no_grad():
        for (batch_x,) in loader:
            batch_x = batch_x.to(device)
            if use_quantized:
                z_q, _ = model.encode_quantized(batch_x, torch.zeros(batch_x.shape[0], device=device))
                all_z.append(z_q.cpu().numpy())
            else:
                z_e = model.encoder(batch_x)
                all_z.append(z_e.cpu().numpy())
...
def refine_metacells_global(
    z_all: np.ndarray,       # (N_cells, latent_dim) latent 特征
    y_all: np.ndarray,       # (N_cells,) 样本类别
    n_metacells: int = 50    # 生成的元细胞数量
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    基于全局 latent 做聚类生成元细胞：
        - 聚类中心作为元细胞 embedding
        - 元细胞类别通过簇内多数投票决定
        - 每个样本映射到最近的元细胞

    返回：
        sample_metacell_ids: (N_cells,) 样本对应元细胞 ID
        metacell_centroids: (n_metacells, latent_dim) 元细胞 embedding
        metacell_labels: (n_metacells,) 元细胞类别
    """
    N, D = z_all.shape

    # ---- KMeans 聚类 ----
    kmeans = KMeans(n_clusters=n_metacells, random_state=0, n_init='auto')
    cluster_labels = kmeans.fit_predict(z_all)
    metacell_centroids = kmeans.cluster_centers_

    # ---- 元细胞类别：多数投票 ----
    metacell_labels = np.zeros(n_metacells, dtype=int)
    for m in range(n_metacells):
        mask = (cluster_labels == m)
        if mask.sum() == 0:
            metacell_labels[m] = -1  # 没有样本
        else:
            metacell_labels[m] = np.bincount(y_all[mask]).argmax()

    # ---- 样本映射到元细胞 ----
    dists = cdist(z_all, metacell_centroids, metric='euclidean')
    sample_metacell_ids = dists.argmin(axis=1)

    return sample_metacell_ids, metacell_centroids, metacell_labels
...
def run_full_pipeline_11way10shot():
    ...
    if os.path.exists(save_path):
        ...
        # 加载权重
        vq_model.load_state_dict(checkpoint["model_state"], strict=False)
        vq_model.eval()
        ...
        torch.save({
            ...
            "config": {
                "input_dim": X_all.shape[1],
                "n_classes": n_classes,
                "codebook_sizes": vq_model.codebook_sizes,
    ...
    new_metacell_ids, metacell_centroids, cid2new = refine_metacells_global(
        z_all=Z_all,
        y_all=y_all,
        n_metacells=3000
    )
    ...
    import collections
    print(collections.Counter(cid2new))
