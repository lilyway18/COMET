...
def load_user_data_new() -> Tuple[np.ndarray, np.ndarray, Dict[int, str]]:
    """
    read RNA data and label from h5ad directly：
        X_all: (N_cells, N_genes) float32
        y_all: (N_cells,) int64, 0~C-1
        class_names: {int -> str}
    label is in obs['cell_type1']
    """
    h5ad_path = 'data/Zheng68k_PBMC_1024.h5ad'
    label_col = "cell_type1"

    print(f"[Load] read h5ad: {h5ad_path}")
    ...
    print(f"[Load] find {len(class_names)} cell type (labels from 0 to {len(class_names)-1})")
    print("[Load] cell type label:")
...
def load_user_data() -> Tuple[np.ndarray, np.ndarray, Dict[int, str]]:
    ...
    read another h5ad，并返回：
        ...
        class_names: {int -> str}
    """
    h5ad_path = "./meltingExp/bmmc/rna.h5ad"
    # h5ad_path = "./meltingExp/bmmc/rna.h5ad"
    label_col = "x"
    label_csv_path = "./meltingExp/bmmc/Label.csv"
    # label_csv_path = "./meltingExp/bmmc/Label.csv"
    ...
    print(f"[Load] X_all.shape={X_all.shape}, y_all.shape={y_all.shape}")
    print(f"[Load] find {len(class_names)} cell type (label from 0 to {len(class_names) - 1})")
    print("[Load] cell type label:")
    for i, name in class_names.items():
        print(f"   id={i:2d} -> {name}")
...
class ClassConditionalVQVAE(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert len(codebook_sizes) == n_classes
    ...
    def _quantize_per_class(
        ...
        device = z_e.device
        B, D = z_e.shape
        z_q = torch.empty_like(z_e)
        code_idx_local = torch.zeros(B, dtype=torch.long, device=device)
        ...
        for c in range(self.n_classes):
            ...
            dist = z_norm2 + e_norm2 - 2.0 * z_c @ emb_c.t()       # (Nc, Kc)

            idx = dist.argmin(dim=1)                # (Nc,)
            z_q_c = emb_c[idx]                      # (Nc, D)
            ...
            if valid.any():
                p = probs[valid]
                usage_loss_terms.append((p * torch.log(p * p.numel())).sum())

        if len(usage_loss_terms) > 0:
            usage_loss = sum(usage_loss_terms) / len(usage_loss_terms)
        else:
            usage_loss = torch.tensor(0.0, device=device)

        return z_q, code_idx_local, usage_loss
...
def eval_supervised_head(
    ...
    if y is None:
        raise ValueError("y must be provided when return_pred=False")
...
def generate_pseudo_labels(
    ...
    print(f"[Pseudo] Generating {n_pseudo_classes} pseudo-classes")
    ...
    print(f"[Pseudo] PCA done: Z_pca.shape = {Z_pca.shape}")

    # ---------- 3. KMeans ----------
    kmeans = KMeans(
        n_clusters=n_pseudo_classes,
        n_init=20,
        max_iter=300,
        random_state=random_state
    )
    pseudo_y = kmeans.fit_predict(Z_pca)

    # ---------- 4. sanity check ----------
    unique, counts = np.unique(pseudo_y, return_counts=True)
    print(f"[Pseudo] Pseudo-class size stats:")
    print(f"  min={counts.min()}, max={counts.max()}, mean={counts.mean():.1f}")

    return pseudo_y, pca, kmeans
...
def run_full_pipeline_11way10shot():
    ...
    '''
    # 生成伪类
    n_true_classes = len(np.unique(y_all))
    n_pseudo_classes = n_true_classes  # 推荐 3~4 倍

    pseudo_y, pca_model, kmeans_model = generate_pseudo_labels(
        X_all,
    ...
    print("Pseudo classes:", np.unique(pseudo_y))
    '''
    ...
    if os.path.exists(save_path):
        ...
        vq_model = ClassConditionalVQVAE(
            ...
            codebook_sizes=model_cfg_loaded["codebook_sizes"],
        ...
        vq_cfg = VQVAETrainingConfig(
            ...
            beta=0.25,
            usage_reg_weight=1e-3,
            device=device,
            target_cells_per_code=15,
        ...
        vq_model = train_vqvae_on_cells(
            X_all, y_all, n_classes, gene_scaler, vq_cfg
        )
        ...
        torch.save({
            ...
            "config": {
                "input_dim": X_all.shape[1],
                "n_classes": n_classes,
                "codebook_sizes": [cb.weight.shape[0] for cb in vq_model.codebooks],
    ...
    from Kmeans_evaluate import evaluate_cell_and_metacell_clustering, plot_clustering_metrics_cell_vs_metacell
    '''
    results = evaluate_cell_and_metacell_clustering(
        X_all=X_all,
        y_all=y_all,
        Z_all=Z_all,
        new_metacell_ids=new_metacell_ids,
        metacell_centroids=metacell_centroids
    )

    print(results)

    plot_clustering_metrics_cell_vs_metacell(results, title='issaac_seq', save_path='clustering_metrics_cell_vs_metacell')
    '''
    '''
    ...
    # --------- 单细胞层面评估（via metacells） ---------
    with torch.no_grad():
        y_meta_pred = eval_supervised_head(
            sup_head,
            metacell_centroids,
            device=DEFAULT_DEVICE,
            return_pred=True
    ...
    y_cell_pred = np.zeros_like(y_all)
    for m in range(len(y_meta_pred)):
        y_cell_pred[new_metacell_ids == m] = y_meta_pred[m]

    acc_cell_via_meta = (y_cell_pred == y_all).mean()
    f1_cell_macro = f1_score(y_all, y_cell_pred, average="macro")  # ← 新增

    print(f"[Cell Acc via Metacell] {acc_cell_via_meta * 100:.2f}%")
    print(f"[Cell Macro-F1 via Metacell] {f1_cell_macro:.4f}")  # ← 新增
    ...
    from draw import plot_cells_and_metacells_tsne, plot_metacells_tsne_only
    '''
    ...
    def save_tsne_data(save_path,
        ...
        print(f"[Save] TSNE data saved to {save_path}")

    save_tsne_data(
        "tsne_data_BMMC_" + str(min_cell) + "min_cell.npz",
        X_all,
        y_all,
        new_metacell_ids,
        metacell_centroids,
    ...
    def save_heads(path, sup_head_cells, sup_head_meta):
        torch.save({
            "sup_head_cells": sup_head_cells.state_dict(),
            "sup_head_meta": sup_head_meta.state_dict()
        }, path)

        print(f"[Save] Heads saved to {path}")

    save_heads("tsne_data_BMMC_" + str(min_cell) + "min_cell.pth", sup_head_cells, sup_head)


    plot_metacells_tsne_only(
        Z_sub_raw=X_all,
        y_sub=y_all,
        sup_head_cells=sup_head_cells,  # 单细胞 head
        sup_head_meta=sup_head,  # 元细胞 head
